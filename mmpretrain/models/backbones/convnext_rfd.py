...
from mmpretrain.workpieces.MBFD import FMBFD, MBFD
from mmpretrain.workpieces.RFD import SRFD, DRFD
...
class ConvNeXt_RFD(BaseBackbone):
    ...
    def __init__(self,
        ...
        # stem = nn.Sequential(
        #     FMBFD(in_channels, self.channels[0]//2),
        #     MBFD(self.channels[0]//2, self.channels[0]),
        # )
        
        stem = SRFD(in_channels, self.channels[0])
        ...
        for i in range(self.num_stages):
            ...
            if i >= 1:
                ...
                # )
                # downsample_layer = nn.Sequential(
                #     MBFD(self.channels[i - 1],channels),
                # )
                downsample_layer = DRFD(self.channels[i - 1], channels)
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            ...
            x = stage(x)
            # print('MBFD output:', x.shape, x.mean().item(), x.std().item())
            if i in self.out_indices:
    ...
    def train(self, mode=True):
        super(ConvNeXt_RFD, self).train(mode)
        self._freeze_stages()
