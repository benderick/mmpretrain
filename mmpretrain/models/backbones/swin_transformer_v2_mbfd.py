...
from ..utils import build_norm_layer as build_norm
...
#     def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, norm_cfg=dict(type='BN2d')):
#         """Initialize Conv layer with given arguments including activation."""
#         super().__init__()
#         self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
#         self.bn = build_norm(norm_cfg, c2)
#         self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
...
# class PTConv(nn.Module):
#     def __init__(self, in_dim, out_dim, k=3, s=2, p=1, d=1, nwa=True, norm_cfg=dict(type='BN2d')):
#         super().__init__()
#         self.conv1 = nn.Conv2d(in_dim//2, out_dim//2, kernel_size=k, stride=s, padding=p, dilation=d, bias=False)
#         self.conv2 = nn.Conv2d(in_dim//2, out_dim//2, kernel_size=k, stride=s, padding=p, dilation=d, bias=False)
#         self.nwa = nwa
#         if nwa:
#             self.norm = build_norm(norm_cfg, out_dim)
#             self.act = nn.SiLU()
...
#         self.conv3 = PTConv(out_channels//2, out_channels//2, k=3, s=2, p=1, norm_cfg=norm_cfg)
#         self.conv4 = DWTConv(in_channels, out_channels//2, norm_cfg=norm_cfg)
#         self.proj_last = Conv(2*out_channels, out_channels, norm_cfg=norm_cfg)
...
#         self.harr = DWTConv(in_channels, out_channels // 2, norm_cfg=norm_cfg)
#         self.proj_last = Conv(2*out_channels, out_channels, norm_cfg=norm_cfg)
...
#                          bias=0.),]):
#         super().__init__(init_cfg=init_cfg)
#         self.FMBFD = FMBFD(in_channels=in_channels, out_channels=embed_dims//4, norm_cfg=norm_cfg)
#         self.MBFD = MBFD(in_channels=embed_dims//4, out_channels=embed_dims, norm_cfg=norm_cfg)
...
class SwinBlockV2Sequence(BaseModule):
    ...
    def __init__(self,
        ...
        if downsample:
            ...
            self.downsample = MBFD_E_V2(**_downsample_cfg)
...
class SwinTransformerV2_MBFD(BaseBackbone):
    """Swin Transformer V2.
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(SwinTransformerV2_MBFD, self).__init__(init_cfg=init_cfg)
        ...
        self.patch_embed = CMBFD_V2(**_patch_cfg)
    ...
    def init_weights(self):
        super(SwinTransformerV2_MBFD, self).init_weights()
    ...
    def train(self, mode=True):
        super(SwinTransformerV2_MBFD, self).train(mode)
        self._freeze_stages()
