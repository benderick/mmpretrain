...
from mmpretrain.workpieces.MBFD import CMBFD, MBFD
        ...
        if stride_q > 1:
            if stride_q == 2:
                if self.dim_mul_in_attention and self.proj is not None:
                    skip_channels = self.out_dims # 如果维度在注意力模块中增加且存在投影，则 skip 通道数为 out_dims
                else:
                    skip_channels = self.in_dims  # 否则，skip 通道数为 in_dims
                self.pool_skip = MBFD(skip_channels, skip_channels)
                print("*********Using MBFD*********")
            else:
...
class MViT_MBFD(BaseBackbone):
    ...
    def __init__(self,
        ...
        self.patch_embed = CMBFD(**_patch_cfg)
